...
export async function timeMachineSearch(query, userId, limit = 10, minScore = 0.3) {
    try {
        ...
        // Map results
        const allResults = searchResult.map((result) => ({
        ...
        const { filteredResults, aiResponse } = await generateTimeMachineResponse(query, topic, allResults, timeRange);
...
async function generateTimeMachineResponse(originalQuery, topic, results, timeRange) {
    if (results.length === 0) {
